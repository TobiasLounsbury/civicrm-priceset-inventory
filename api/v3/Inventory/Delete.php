...
function civicrm_api3_inventory_delete($params) {
    if (array_key_exists('sid', $params) || array_key_exists('id', $params)) {
        if (array_key_exists('id', $params) && ($params['field_id'] || $params['field_id'] == 0)) {
            $sql = "select `Quantity` FROM `{PSI_TABLE}` WHERE `id` = {$params['id']} LIMIT 1";
        } elseif (array_key_exists('field_id', $params) && $params['field_id']) {
            if(!array_key_exists('field_value_id', $params)) {
                $sql = "select `Quantity`,`field_value_id` FROM `{PSI_TABLE}` WHERE `field_id` = {$params['field_id']} LIMIT 1";
            } else {
                $sql = "select `Quantity`,`field_value_id` FROM `{PSI_TABLE}` WHERE `field_id` = {$params['field_id']} AND `field_value_id` = {$params['field_value_id']} LIMIT 1";
            }
        } else {
